  ...
  return (
    <section className="glass-card rounded-2xl p-5 space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h2 className="text-sm font-medium text-[var(--color-brand-text-secondary)] uppercase tracking-wider">Look & Feel</h2>
      </div>

      <div className="flex items-center justify-between">
        <Label className="text-sm text-white">Show cents on the dashboard</Label>
        <Switch
      ...
      {showInstallRow ? (
        <div className="pt-2 border-t border-[var(--color-brand-border)]">
          <p className="text-xs text-[var(--color-brand-text-muted)] mb-2">Get the desktop app</p>
          {isInstalled ? (
            <p className="text-sm font-medium text-emerald-400">You&apos;re all set — app installed ✓</p>
          ) : (
            <button
              type="button"
            ...
            >
              ...
              Install Buddget on your desktop
            </button>
          )}
        </div>
      ) : null}
