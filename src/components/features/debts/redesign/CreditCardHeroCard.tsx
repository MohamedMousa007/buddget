  ...
  return (
    <div className="relative flex flex-col rounded-[18px] p-4 text-white" style={heroCardStyle(vm.color)}>
      ...
      <div className="mt-4 flex items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 flex items-baseline gap-2">
            <span className="font-mono-numbers text-[34px] font-bold leading-none tracking-[-1px]">{fmtWhole(vm.outstanding)}</span>
            <span className="text-sm text-white/55">{vm.currency}</span>
          </p>
        </div>
        {vm.utilPct != null ? (
          <div className="shrink-0 pb-1 text-end">
            <p className="font-mono-numbers text-[13px] font-bold" style={{ color: over ? '#FF6B6B' : tone }}>
              {vm.utilPct}% used
            </p>
            {over ? (
              <p className="mt-0.5 font-mono-numbers text-[12px] text-white/55">over by {fmtWhole(vm.overBy)}</p>
            ) : vm.limit != null ? (
              <p className="mt-0.5 font-mono-numbers text-[12px] text-white/45">{fmtWhole(Math.max(0, vm.limit - vm.outstanding))} free</p>
            ) : null}
          </div>
        ) : null}
      </div>
      ...
      <div className="mt-3.5 grid grid-cols-3 gap-2">
        <div className="rounded-[12px] border border-white/10 bg-white/[.03] px-3 py-2.5">
          <p className="text-[10px] font-bold uppercase tracking-[0.06em] text-white/45">
            {vm.due ? `Due ${format(parseISO(vm.due), 'MMM d')}` : 'Due'}
          </p>
          <p className="mt-1 font-mono-numbers text-[15px] font-bold leading-tight">
            {vm.daysLeft != null ? `${vm.daysLeft} days left` : '—'}
          </p>
        </div>
        <div className="rounded-[12px] border border-white/10 bg-white/[.03] px-3 py-2.5">
          <p className="text-[10px] font-bold uppercase tracking-[0.06em] text-white/45">All due</p>
          <p className="mt-1 font-mono-numbers text-[15px] font-bold leading-tight">{fmtWhole(vm.allDue)}</p>
          {vm.installmentDue > 0 ? (
            <p className="mt-0.5 text-[10px] font-semibold text-[#2CE0C6]">incl. {fmtWhole(vm.installmentDue)}</p>
          ) : null}
        </div>
        <div className="rounded-[12px] border border-white/10 bg-white/[.03] px-3 py-2.5">
          <p className="text-[10px] font-bold uppercase tracking-[0.06em] text-white/45">After payment</p>
          <p className="mt-1 font-mono-numbers text-[15px] font-bold leading-tight">{fmtWhole(vm.after)}</p>
        </div>
      </div>
