...
export function BuddgyStepSavings({ flow }: { flow: BuddgyFlowApi }) {
  const { maxSavings, savingsAmount, setSavingsAmount, settings } = flow
  ...
  const onSlider = useCallback(
    ...
    [maxSavings, setSavingsAmount]
  )

  const onNext = async () => {
    flow.saveSavings()
    await flow.loadAiFillRows()
    flow.setStep('aiFill')
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => setSavingsAmount(Math.round(maxSavings))}
          className="cursor-pointer rounded-full border border-[#2A2A38] bg-[#1A1A24] px-4 py-2 text-sm text-white hover:bg-[#1A1A24]/90"
        >
          Maximum
        </button>
        <button
          type="button"
          onClick={() => setSavingsAmount(Math.round(maxSavings * 0.25))}
          className="cursor-pointer rounded-full border border-[#2A2A38] bg-[#1A1A24] px-4 py-2 text-sm text-white hover:bg-[#1A1A24]/90"
        >
          Custom
        </button>
