/**
 * SmsTrackingGuide.tsx — in-app SMS auto-tracking onboarding guide.
 * Faithful port of the approved design (`design/sms-guide/`). The STEPS array,
 * per-step annotation coordinates, and inline styles are the source of truth and
 * must not drift. Copy is sourced from i18n (`smsTracking.guide`); the English
 * values are byte-identical to the approved prototype.
 */
'use client';
...
// Titles + descriptions live in i18n (`smsTracking.guide.steps`), index-aligned.
const STEPS: Step[] = [
...
// ── Messages glyph (brand-specific; not a Lucide chrome icon) ────────────────
...
function Hero() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%', padding: '8px 0', animation: 'bg-heroFloat 5.5s ease-in-out infinite' }}>
      ...
      {/* parsed expense (static illustration; MoneyDisplay for the amount) */}
        ...
        <div style={{ width: 780, height: 844, display: 'flex', willChange: 'transform', transform: `translateX(${-(step - 1) * 390}px)`, transition: 'transform .44s cubic-bezier(.4,0,.2,1)' }}>
          ...
          <section data-screen-label="Guided setup" style={{ width: 390, height: 844, flexShrink: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
            ...
            <div style={{ flex: 1, padding: '2px 16px 0', minHeight: 0, display: 'flex' }}>
              {cur.concept ? <KeywordScreen onCopy={onCopy} /> : <PhoneMock step={cur} title={curCopy.title} />}
            </div>
            ...
            <div style={{ padding: '10px 18px 6px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <button onClick={() => setSubStep(s => Math.max(0, s - 1))} aria-label={g.ariaPrev} style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--color-brand-card)', border: '1px solid var(--color-brand-border)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <ChevronLeft size={16} color={safe > 0 ? '#9898B0' : '#5A5A72'} />
              </button>
              ...
              <button onClick={() => setSubStep(s => Math.min(total - 1, s + 1))} aria-label={g.ariaNext} style={{ width: 38, height: 38, borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', background: isLast ? RED : 'var(--color-brand-card)', border: `1px solid ${isLast ? 'transparent' : 'var(--color-brand-border)'}` }}>
