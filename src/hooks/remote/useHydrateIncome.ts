...
export function useHydrateIncome(): void {
  ...
  useEffect(() => {
    ...
    if (useFinanceStore.getState().profile.id === uid) { markHydrated(uid, 'income'); return }
    let cancelled = false
    const supabase = createClient()

    ;(async () => {
      try {
        const res = await supabase.from('income_sources').select('*').eq('user_id', uid).is('deleted_at', null)
        if (cancelled) return
        if (res.data) {
