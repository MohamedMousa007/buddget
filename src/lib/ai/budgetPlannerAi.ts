...
import { effectivePlanCategoryAmount, totalPlannedExpensesForPlan } from '@/lib/budget/budgetPlans'
import { parseModelJsonToAIResponse, type AIResponse } from '@/lib/ai/gemini'

const EVAL_RATINGS = ['Realistic', 'Tight', 'Needs Adjustment'] as const
...
function normalizeRating(raw: string): BudgetPlanEvalRating {
  ...
  if (lower.includes('adjust') || lower.includes('risk')) return 'Needs Adjustment'
  return 'Realistic'
}

/** Serializable snapshot for prompts (ids preserved for AI apply actions). */
export function buildPlanRowsForPrompt(plan: BudgetPlan): string {
  const rows = plan.categories.map((c) => ({
    ...
    effectiveTotal: effectivePlanCategoryAmount(c),
  }))
...
export function buildBudgetPlannerContextBlock(
  ...
  const planned = totalPlannedExpensesForPlan(plan)
  ...
  return [
    ...
    `TOTAL_PLANNED_EXPENSES (sum of category effective totals): ${planned}`,
    `PROJECTED_SAVINGS (income - planned): ${projectedSavings}`,
    'PLAN_ROWS_JSON:',
    buildPlanRowsForPrompt(plan),
  ].join('\n')
}

export async function evaluateBudgetPlanWithAi(contextBlock: string): Promise<BudgetPlanEvalResult> {
  const instruction = `You are a budgeting coach. Read the user's monthly budget plan (JSON below) and monthly income.

${contextBlock}

Return ONLY a single JSON object, no markdown, no code fences:
{"rating":"Realistic"|"Tight"|"Needs Adjustment","explanation":"2-3 short sentences in plain language"}
...
- Needs Adjustment: planned expenses exceed income or allocations are unrealistic.`
  ...
  try {
    ...
    if (!parsed.success) {
      return { rating: 'Realistic', explanation: text.slice(0, 400) }
    }
    return {
...
export function buildBudgetPlannerChatSystemPrompt(contextBlock: string): string {
  return `You are Buddget AI on the Budget Planner page. Help the user adjust category amounts, suggest cuts, evaluate realism, and explain projections.
