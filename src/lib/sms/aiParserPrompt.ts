...
- LEDGER SIGN BEATS WORDING — apply this before anything else. When the amount carries a trailing sign ("USD 3,097.24+", "EGP 715.00-") that sign IS the direction, because it is the bank's own ledger marker: "+" = money IN, "-" = money OUT. In this statement format the account named after "to"/"from" is the account being credited or debited, NOT a counterparty — so "TT Payment to 103-104***-110 USD 3,097.24+" is money ARRIVING (kind "income"), and "VODAFONE EGYPT Purchase from 103-104***-001 EGP 250.00-" is money LEAVING (kind "purchase"). Never return an outgoing kind for a "+" amount or an incoming kind for a "-" amount.
- WIRES: "TT", "TT Payment", "Telegraphic Transfer", "SWIFT", "Wire transfer", "Remittance received", "حوالة واردة" = an international bank wire. Inbound (sign "+", or "received"/"credited"/"واردة") is "income" — this is how salaries paid from abroad, payroll platforms (Deel, Wise, Payoneer, Remote, Upwork) and family remittances arrive. Outbound (sign "-", or "sent"/"debited") is "instant_transfer_out".
