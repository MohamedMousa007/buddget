...
export const TXN_KEYWORD_RE =
  /(?:debit(?:ed)?|credit(?:ed)?|withdraw(?:n|al)?|charged|spent\s+at|purchase\s+(?:of|at)|pos\s+purchase|atm\s+(?:withdrawal|cash)|transferred|payment\s+(?:of|received)|received\s+from|paid\s+to|تم\s*خصم|تم\s*سحب|تم\s*دفع|تم\s*إيداع|تم\s*استلام|تم\s*تحويل|تم\s*إضافة|عملية\s*شراء|سحب\s*نقدي|نقاط\s*بيع)/i
